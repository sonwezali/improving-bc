...
def create_box(root, pos, quat, size, width, rgba, friction=[0.5, 0.005, 0.0001],
    ...
    if lid_type == "hinge":
        lid.add("joint", type="hinge", axis=[1, 0, 0], range=[0, np.pi],
                pos=[0, -(size[1]+width/2), 0], name="lid_joint")
        lid.add("geom", type="box", size=[size[0]+width, size[1]+width, width/2],
                rgba=[rgba[0]*0.8, rgba[1]*0.8, rgba[2]*0.8, 1], friction=friction, pos=[0, 0, 0],
                mass=0.025)
        # Graspable handle: a single tall post, thin along y so the gripper pads
        # (which close along y) can clamp it, and wide along x to match the pad
        # face. No wide cap on top: a cap wider than the post would rest on the
        # pads and stop the gripper from descending to grip height. High friction
        # lets the closed grip hold while lifting the lid open.
        handle_y = size[1] - 0.015  # set back from the front wall to clear fingers
        lid.add("geom", type="box", size=[0.011, 0.004, 0.035],
                rgba=[0.85, 0.85, 0.85, 1.0], friction=[2.0, 0.05, 0.001],
                pos=[0, handle_y, width/2 + 0.035], mass=0.01)
